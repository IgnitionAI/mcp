import createLinkedIn, { LinkedIn, LinkedInClient } from '../lib/linkedin';
import { getAccessToken as getAuthToken } from './auth';
...
export async function loginLinkedIn(redirectUriParam: string = redirectUri) {
    try {
        const linkedin = getLinkedInInstance();
        if (redirectUriParam) {
            linkedin.auth.setCallback(redirectUriParam);
        }
